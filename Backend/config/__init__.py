...
from config.global_constants import VERSION, APP_NAME
...
class DynamicConfig(Generic[T]):
    def __init__(self, name: str, model_class: type[T]):
        ...
        self.template_path = os.path.join(os.path.dirname(__file__), "templates", f"{name}.yaml")
        ...
        if os.path.exists(self.template_path) and not os.path.exists(self.local_path):
            shutil.copy2(self.template_path, self.local_path)
        ...
        self._file_handler.add_watched_file(self.template_path)
        ...
        self._observer.schedule(self._file_handler, self.template_path, recursive=False)
